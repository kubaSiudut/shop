  ...
  reducers: {
    ...
    clear: (state, action) => {
      ...
      localStorage.setItem(
        "CartCustomizeItems",
        JSON.stringify(state.customizedItems)
      );
    },
    updateFromLocalStorage: (state, action) => {
      ...
      if (localStorage.getItem("CartCustomizeItems")) {
        state.customizedItems = localStorage.getItem("CartCustomizeItems");
        state.customizedItems = JSON.parse(state.customizedItems);
      }
      state.totalAmountcarpentery = JSON.parse(
        localStorage.getItem("carpenteryCartPriceTotal")
      );
    ...
    addToCarpenterShopCart: (state, action) => {
      ...
      state.totalAmountcarpentery +=
        action.payload.item.unitPrice * action.payload.item.amount;
    },
    addItem: (state, action) => {
      console.log("dodaje produkt do koszyka");
      if (
        state.items &&
        state.items.find((el) => el.id === action.payload.id)
      ) {
        let indeex = state.items.findIndex((el) => el.id === action.payload.id);

        state.items[indeex].amount =
          state.items[indeex].amount * 1 + action.payload.amount * 1;
    ...
    addCustomizedItem: (state, action) => {
      ...
      if (
        state.customizedItems &&
        state.customizedItems.find((el) => el.id === tmpId)
      ) {
        let indeex = state.customizedItems.findIndex((el) => el.id === tmpId);

        state.customizedItems[indeex].amount =
          state.customizedItems[indeex].amount * 1 + action.payload.amount * 1;
    ...
    modifyAmounCustomizedProdInCart: (state, action) => {
      let indeex = state.customizedItems.findIndex(
...
export const {
  ...
  addToCarpenterShopCart,
  addItem,
  modifyAmounInCart,
  modifyAmounCustomizedProdInCart,
  addCustomizedItem,
} = cartSlice.actions;
